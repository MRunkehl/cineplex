/*
 * Jellyfin Slideshow by M0RPH3US --- with modifications by prism2001 v3.1.0
 */
...
const CONFIG = {
    ...
    shuffleInterval: 15000,   // how long until the next slide appears
    retryInterval: 500,
    minSwipeDistance: 50,
    loadingCheckInterval: 100,
    maxPlotLength: 700,       // maximum synopsis length before it gets truncated
    maxMovies: 25,
    maxTvShows: 25,
    maxItems: 500,
...
const ApiUtils = {
    ...
    /**
    * Fetch item IDs from the list file (with cache control)
    ...
    */
    async fetchItemIdsFromList() {
        try {
            ...
            console.log(`Fetching list.txt with cache control: ${listFileName}`);
            ...
            if (!response.ok) { //
                console.warn(`list.txt fetch failed with status ${response.status}. Using random items.`);
                return [];
            }
            ...
            console.log(`Successfully fetched and processed ${ids.length} IDs from list.txt`);
            return ids;
        } catch (error) {
            console.error("Error fetching or processing list.txt:", error);
            return [];
        }
    },
...
class SlideTimer {
     ...
     */
    stop() {
        console.log("Interval stopped: " + this.timerId);
        if (this.timerId) {
    ...
    start() {
        ...
        console.log("Interval started: " + this.timerId);
        return this;
    }
     ...
     */
    restart() {
        console.log("Interval restarted: " + this.timerId);
...
const SlideCreator = {
    /**
       ...
       * @returns {HTMLElement|null} Slide element or null on error
       */
    createSlideElement(item, name) {
        ...
        const backdrop = SlideUtils.createElement("img", {
            ...
            src: `${serverAddress}/Items/${itemId}/Images/Backdrop/0?quality=100`,
            alt: "Backdrop",
            loading: "eager",
        });
        ...
        const rawOverview = (item.Overview.length > CONFIG.maxPlotLength ? item.Overview.substring(0, CONFIG.maxPlotLength-3) + "..." : item.Overview) || "No overview available";
        const htmlOverlay = marked.parse(rawOverview)
        const plotElement = SlideUtils.createElement("div", {
            className: "plot",
        });

        plotElement.innerHTML = `<div class="marquee-vertical"><div class="marquee-inner">${htmlOverlay}</div></div>`;
        ...
        setTimeout(() => {
            ...
            if (scrollAmount > 0) {
                const durationSeconds = scrollAmount * 0.2 + 6;

                // Full reset
                inner.style.animation = "none";
                inner.style.removeProperty("animation");
                inner.style.removeProperty("animationDelay");
                inner.style.transform = "translateY(0)";
                inner.style.setProperty('--scroll-amount', `${-scrollAmount}px`);

                // Ensure reset takes effect first, THEN re-apply animation
                requestAnimationFrame(() => {
                    requestAnimationFrame(() => {
                        inner.style.animation = `scroll-vertical ${durationSeconds}s linear infinite`;
                        inner.style.animationDelay = "5s";
                    });
                });
            } else {
                // Reset transform if no scroll needed
                inner.style.animation = "none";
                inner.style.transform = "translateY(0)";
            }
        }, 0);
    ...
    createRatingInfo(item) {
        ...
        if (season === undefined) {
            ...
            runTimeElement.textContent = `Endet um ${formattedEndTime}`;
        } else {
    ...
    async createSlideForItemId(itemId) {
        try {
            // If we've already created this slide, just return it
            ...
            if (!item) {
                console.error(`No details for item ${itemId}`);
                return null;
            }

            // Pass the actual Jellyfin "Name" field into the slide
            const slideElement = this.createSlideElement(item, item.Name);
...
const SlideshowManager = {
    ...
    async updateCurrentSlide(index) {
        ...
        try {
            ...
            if (totalItems === 0) {
                console.warn("No items to display in slideshow.");
                STATE.slideshow.isTransitioning = false;
                return;
            }
            // Use modulo to wrap around index safely
            ...
            if (!currentSlide) { //
                ...
                if (!currentSlide) {
                    ...
                    STATE.slideshow.isTransitioning = false;
                    // Attempt to recover by moving to the next potential slide after a delay
                    setTimeout(() => this.nextSlide(), 500);
                    return;
                }
            }

            // Remove active and paused classes from the previous slide
            ...
            // If the slideshow is currently paused, add the paused class to the new active slide
            if (STATE.slideshow.isPaused) {
            ...
            // Clean up classes on non-active slides after transition
            setTimeout(() => {
            ...
            // Restart timer only if not paused
    ...
    /**
       * Loads slideshow data and initializes the slideshow (incorporating new config)
       */
...
/**
 * Initializes the pause/play button for the slideshow (Revised)
 */
const initPauseButton = () => {
    ...
    pauseButton.addEventListener("click", (e) => {
        ...
        // Find the currently active slide *at the time of click*
        ...
        if (STATE.slideshow.isPaused) {
            ...
            // Add pause class only to the currently active slide
            ...
            // Resume actions
            if (STATE.slideshow.slideInterval) {
                // Just restart the timer. It will fire after the interval.
                STATE.slideshow.slideInterval.restart();
            }
            pauseButton.innerHTML = '<i class="material-icons">pause</i>';
            pauseButton.title = "Pause Slideshow";
            // Remove pause class only from the currently active slide
            if (currentActiveSlide) {
